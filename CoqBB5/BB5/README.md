...
We recommend installing `coq-native` before compiling the proof as it compiles about three times faster (`coq-native` currently requires using OCaml 4 instead of newer versions, e.g. `opam switch 4.14.2`):
...
Then, compiling `CoqBB5/BB5` takes about 45 minutes on 13 cores (Apple silicon) and using Coq's `native_compute`.
