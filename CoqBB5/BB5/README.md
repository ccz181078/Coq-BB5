...
We recommend installing `coq-native` before compiling the proof as it will make the proof compile about three times faster (`coq-native` currently requires using OCaml 4 instead of newer versions, e.g. `opam switch 4.14.2`):
...
In the above, compiling BusyCoq first (see `../../BusyCoq`) is needed in order to get the proofs for 12 out the 13 [Sporadic Machines](#sporadic-machines). BusyCoq takes about 8 minutes on 13 cores (Apple silicon).

#### Compile time 

Compiling the proof takes about 45 minutes on 13 cores (Apple silicon) and using Coq's `native_compute` (see above).
