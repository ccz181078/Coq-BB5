(** 
  This files contains the theorem statement of "BB(2,4) = 38" as well as the definitions needed for the statement.
  The statement (see `BB2x3_value_statement`) and the definitions are quite transparent, the only prerequisite is to know what Turing machine are.
*)

Require Import ZArith.

(** 2-state Turing machines *)
Inductive St:Set :=
| St0
| St1.

(** 3-symbol Turing machines *)
Inductive Σ:Set :=
| Σ0
| Σ1
| Σ2.
...
Definition BB2x3 := 38%N.

(** Main theorem statement: BB(2,4) = 38. The statement is split into two parts:
  
  - Upper bound (hard): For all 2-state 3-symbol Turing machine `tm` and natural number `n0`, if `tm` halts at `n0` steps, then `n0` is less than or equal to 38.
  - Lower bound (easy): There exists a Turing machine `tm` that halts at 38 steps.
*)
Definition BB2x3_value_statement :=
  (forall tm n0, HaltsAtPlusOne Σ tm n0 (InitES Σ Σ0) -> n0 <= N.to_nat BB2x3) /\
  (exists tm, HaltsAtPlusOne Σ tm (N.to_nat BB2x3) (InitES Σ Σ0)).