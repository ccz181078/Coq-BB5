(** 
  This files contains the theorem statement of "BB(3) = 6" as well as the definitions needed for the statement.
  The statement (see `BB2_value_statement`) and the definitions are quite transparent, the only prerequisite is to know what Turing machine are.
*)
...
(** Main theorem statement: BB(3) = 6. The statement is split into two parts:
  
  - Upper bound (hard): For all 2-state 2-symbol Turing machine `tm` and natural number `n0`, if `tm` halts at `n0` steps, then `n0` is less than or equal to 6.
  - Lower bound (easy): There exists a Turing machine `tm` that halts at 6 steps.
*)
Definition BB2_value_statement :=
  (forall tm n0, HaltsAtPlusOne Σ tm n0 (InitES Σ Σ0) -> n0 <= N.to_nat BB2) /\
  (exists tm, HaltsAtPlusOne Σ tm (N.to_nat BB2) (InitES Σ Σ0)).